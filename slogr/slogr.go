...
func (sink slogSink) WithValues(kvList ...any) logr.LogSink {
...
func (sink slogSink) Info(level int, msg string, kvList ...any) {
	args := make([]any, 0, len(kvList)+4)
	if len(sink.name) != 0 {
...
func (sink slogSink) Error(err error, msg string, kvList ...any) {
	args := make([]any, 0, len(kvList)+4)
	if len(sink.name) != 0 {
		args = append(args, "logger", sink.name)
	}
...
func (sink slogSink) log(level slog.Level, msg string, kvList ...any) {
